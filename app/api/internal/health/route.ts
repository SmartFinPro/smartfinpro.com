...
// INTERNAL Health & Readiness endpoint — full diagnostics, Bearer-token protected.
//
// GET /api/internal/health                — full check (Supabase, env, cron freshness, memory)
// GET /api/internal/health?quick=1        — liveness only (no DB call, instant response)
//
// Auth: Authorization: Bearer <HEALTH_TOKEN>
//   HEALTH_TOKEN must be set in .env.local (required — route returns 500 if missing).
//
// Usage from monitoring services (BetterStack, UptimeRobot):
//   curl -H "Authorization: Bearer $HEALTH_TOKEN" https://smartfinpro.com/api/internal/health
//
// Public liveness (no detail) remains available at /api/health.
...
function unauthorized(): NextResponse {
  // Minimal 401 — no hint about whether the token is misconfigured vs. wrong.
  return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
}

export async function GET(request: NextRequest): Promise<NextResponse> {
  // ── Auth gate (required, timing-safe) ───────────────────────────────────
  const healthToken = process.env.HEALTH_TOKEN;
  if (!healthToken || healthToken.startsWith('your-')) {
    logger.error('[internal/health] HEALTH_TOKEN not configured — endpoint disabled');
    return NextResponse.json(
      { error: 'Endpoint disabled — HEALTH_TOKEN not configured' },
      { status: 503 },
    );
  }

  if (!validateBearer(request.headers.get('authorization'), healthToken)) {
    return unauthorized();
  }
  ...
  if (overallStatus === 'down') {
    ...
    if (now - lastOutageAlertAt > ALERT_COOLDOWN_MS) {
      ...
      const alertMsg = [
        `\u{1F534} <b>SMARTFIN OUTAGE DETECTED</b>`,
        ``,
      ...
      sendTelegramAlert(alertMsg).catch((err) => {
        logger.error('[internal/health] Telegram outage alert failed', {
