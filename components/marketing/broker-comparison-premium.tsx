...
const regColors: Record<string, string> = {
  ...
  'ASIC': 'bg-emerald-500/15 text-emerald-400 border-emerald-500/30',
  ...
  return (
    <div className={wrapperClass}>
      ...
      <div className="hidden lg:block">
        ...
        <div
          className="grid grid-cols-[2fr_1fr_1fr_1fr_1.5fr_0.8fr_1fr] gap-0 bg-slate-950 border-b border-[rgba(255,255,255,0.06)] relative z-10"
        >
        ...
        {filteredBrokers.map((broker, index) => (
          <div key={broker.slug}>
            ...
            {broker.isEditorsChoice && (
              <div className="bg-gradient-to-r from-emerald-500/10 via-emerald-500/5 to-transparent border-b border-emerald-500/20 px-4 py-1.5 flex items-center gap-2">
            ...
            <div
              className={`grid grid-cols-[2fr_1fr_1fr_1fr_1.5fr_0.8fr_1fr] gap-0 items-center transition-colors hover:bg-white/[0.02] cursor-pointer ${
            ...
            >
              ...
              <div className="px-4 py-4">
                <div className="flex flex-col gap-1">
                  ...
                  <span className="text-xs text-slate-500">
                    {broker.rating}/5
                    {broker.reviewCount && ` (${broker.reviewCount.toLocaleString()})`}
                  </span>
                </div>
              </div>
              ...
              {/* Leverage */}
              <div className="px-4 py-4">
                <div className="text-sm text-white font-medium">{broker.leverage}</div>
                {broker.leveragePro && (
              ...
              {/* Spreads */}
              <div className="px-4 py-4">
                <div className="text-sm text-emerald-400 font-medium">{broker.spreads}</div>
              ...
              {/* Min Deposit */}
              <div className="px-4 py-4">
                <div className="text-sm text-white font-medium">{broker.minDeposit}</div>
              </div>
              ...
              <div className="px-4 py-4" onClick={(e) => e.stopPropagation()}>
                <Button
                  ...
                  className={`w-full gap-1.5 text-xs ${
                    broker.isEditorsChoice
                      ? 'bg-emerald-500 hover:bg-emerald-600 text-white border-0 shadow-sm shadow-emerald-500/25 btn-shimmer'
                  ...
                  {broker.pros && (
                    <div className="space-y-2 mb-3">
                      {broker.pros.map((pro, i) => (
                        <div key={i} className="flex items-start gap-2.5 rounded-lg px-3 py-2 bg-emerald-500/[0.06] border border-emerald-500/15">
      ...
      <div className="lg:hidden space-y-4 p-4">
        {filteredBrokers.map((broker, index) => (
          ...
          >
            {/* Editor's Choice Banner */}
            {broker.isEditorsChoice && (
              <div className="bg-gradient-to-r from-emerald-500/15 to-transparent px-4 py-2 flex items-center gap-2 border-b border-emerald-500/20">
                <Award className="h-3.5 w-3.5 text-emerald-400" />
                <span className="text-xs font-semibold text-emerald-400 uppercase tracking-wider">
                  Best Overall 2026
                </span>
            ...
            <div className={`p-4 ${glass ? 'bg-white/[0.02]' : 'bg-slate-900/50'}`}>
              ...
              <div className="grid grid-cols-3 gap-3 mb-4">
                <div className={`${glass ? 'bg-white/[0.03]' : 'bg-slate-800/50'} rounded-lg p-2.5 text-center`}>
                  ...
                  <div className="text-sm text-emerald-400 font-semibold">{broker.spreads}</div>
                </div>
                <div className={`${glass ? 'bg-white/[0.03]' : 'bg-slate-800/50'} rounded-lg p-2.5 text-center`}>
                  <div className="text-[10px] text-slate-500 uppercase tracking-wider mb-1">Leverage</div>
                  <div className="text-sm text-white font-semibold">{broker.leverage}</div>
                </div>
                <div className={`${glass ? 'bg-white/[0.03]' : 'bg-slate-800/50'} rounded-lg p-2.5 text-center`}>
                  <div className="text-[10px] text-slate-500 uppercase tracking-wider mb-1">Min. Dep.</div>
                  <div className="text-sm text-white font-semibold">{broker.minDeposit}</div>
                </div>
              </div>
              ...
              {expandedMobile === index && (
                <div className="mb-4 space-y-2">
                  {broker.verdict && (
                    <div className="flex items-start gap-2 text-xs text-slate-200 font-medium bg-emerald-500/5 rounded-lg px-3 py-2 border border-emerald-500/10">
                      <Sparkles className="h-3.5 w-3.5 text-emerald-400 shrink-0 mt-0.5" />
                      {broker.verdict}
                    </div>
                  )}
                  {broker.pros?.map((pro, i) => (
                    <div key={i} className="flex items-center gap-2 text-xs text-slate-300 bg-emerald-500/[0.04] rounded-lg px-3 py-2">
                      <CheckCircle className="h-3.5 w-3.5 text-emerald-400 shrink-0" /> {pro}
                    </div>
                  ))}
                  {broker.con && (
