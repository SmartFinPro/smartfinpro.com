...
export async function detectFraud(params: {
  ...
  // by VN/BR/FR + a long single-click tail) and is not monetizable. Flag it so
  ...
  //
  // NOTE: `head: true` makes postgrest-js issue an HTTP HEAD request — it never
  // parses a response body, so `data` is ALWAYS null. The answer lives in
  // `count`. Reading `data` here is what left this heuristic dead in production
  // (2394 clicks, zero duplicate_ip flags). Covered by
