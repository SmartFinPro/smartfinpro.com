...
// intelligentinvesting.ai platform migration, disclosed and ranked last, not
...
export const caAiToolsFinanceConfig: TopicConfig = {
  ...
  compareRows: [
    ...
    { key: 'pricingModel', label: 'Pricing model', accessor: (p) => PRICING_MODEL_LABEL[attrStr(p, 'pricing_model')] ?? 'N/A' },
    { key: 'segment', label: 'Best for', accessor: (p) => SEGMENT_LABEL[attrStr(p, 'target_segment')] ?? 'N/A' },
  ...
  detailRows: [
    { key: 'priceNote', label: 'Pricing detail', accessor: (p) => attrStr(p, 'starting_price_note') || 'N/A' },
    { key: 'aiFeatures', label: 'AI features', accessor: (p) => attrStr(p, 'ai_features_note') || 'N/A' },
    { key: 'reviewNote', label: 'Review detail', accessor: (p) => attrStr(p, 'review_note') || 'N/A' },
  ...
  buyerGuide: [
    ...
    },
    {
      ...
      body: 'Only KOHO publishes native CAD pricing. ChatGPT, Perplexity, Monarch Money and Copilot Money all bill in USD, with your card issuer applying currency conversion at checkout, so the CAD figures shown here are estimates, not fixed prices. Confirm the actual charge on your statement before assuming the CAD estimate is exact.',
    },
    {
  ...
  faq: [
    ...
    },
    {
      ...
      a: 'Only KOHO publishes native CAD pricing. ChatGPT, Perplexity, Monarch Money and Copilot Money all bill in USD, so the CAD prices shown here are estimated conversions, and your actual charge depends on your card issuer\'s exchange rate at the time of billing.',
    },
    {
    ...
    },
    {
      ...
      a: 'Every price, feature and regulatory disclosure on this page was researched against official vendor sources on 11 July 2026. AI features in this category are shipping fast, so confirm current pricing and terms on the vendor\'s own site before subscribing.',
    },
  ],
  compliance: {
    notice:
