...
export const caForexBrokersConfig: TopicConfig = {
  ...
  compareRows: [
    ...
    { key: 'platforms', label: 'Platforms', accessor: (p) => attrArr(p, 'platforms').join(', ') || 'N/A', score: (p) => attrArr(p, 'platforms').length },
    { key: 'minDeposit', label: 'Min. deposit', accessor: (p) => (p.accountMinimum ? cad(p.accountMinimum) : 'No minimum'), score: (p) => -p.accountMinimum },
    { key: 'ciro', label: 'CIRO dealer member', accessor: (p) => attrStr(p, 'ciro_member') || 'N/A' },
  ],

  detailRows: [
    { key: 'accountNote', label: 'Account type (spread/commission basis)', accessor: (p) => attrStr(p, 'account_type_note') || 'N/A' },
    { key: 'trustpilotNote', label: 'Rating source note', accessor: (p) => attrStr(p, 'trustpilot_note') || 'N/A' },
