...
export const caCybersecuritySmbConfig: TopicConfig = {
  ...
  compareRows: [
    { key: 'category', label: 'Category', accessor: (p) => CATEGORY_LABEL[attrStr(p, 'product_category')] ?? 'N/A' },
    { key: 'price', label: 'Starting price', accessor: (p) => cad(p.monthlyFee) },
    { key: 'feature', label: 'Key SMB feature', accessor: (p) => attrStr(p, 'key_feature_note') || 'N/A' },
    { key: 'caPresence', label: 'CA presence', accessor: (p) => attrStr(p, 'ca_presence_note') || 'N/A' },
    {
      key: 'rating',
      label: 'Review score',
      accessor: (p) => {
  ...
  faq: [
    ...
    },
    {
      ...
      a: 'Every price, feature and security disclosure on this page was researched against official vendor sources, G2, Capterra and security-incident reporting on 11 July 2026. Pricing and incident status can change, so confirm current terms on the vendor\'s own site before subscribing.',
    },
  ],
  compliance: {
    notice:
