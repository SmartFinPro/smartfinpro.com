...
export const caGoldInvestingConfig: TopicConfig = {
  ...
  compareRows: [
    { key: 'productType', label: 'Product type', accessor: (p) => PRODUCT_LABEL[attrStr(p, 'product_type')] ?? 'N/A' },
    { key: 'premium', label: 'Premium over spot', accessor: (p) => pctOrDash(attrNumOrNull(p, 'premium_over_spot_pct')) },
    { key: 'storage', label: 'Storage', accessor: (p) => attrStr(p, 'storage_note') || 'N/A' },
    { key: 'registered', label: 'RRSP/TFSA eligibility', accessor: (p) => attrStr(p, 'registered_account_note') || 'N/A' },
    { key: 'buyback', label: 'Buyback', accessor: (p) => yesNo(attrBool(p, 'buyback_available')), score: (p) => (attrBool(p, 'buyback_available') ? 1 : 0) },
    { key: 'accreditation', label: 'Accreditation', accessor: (p) => ACCRED_LABEL[attrStr(p, 'accreditation')] ?? 'N/A', score: (p) => (attrStr(p, 'accreditation') === 'government_mint' ? 3 : attrStr(p, 'accreditation') === 'osc_regulated_fund' ? 2 : attrStr(p, 'accreditation') === 'lbma_affiliate' ? 1 : 0) },
  ...
  detailRows: [
    { key: 'premiumNote', label: 'Premium / fee detail', accessor: (p) => attrStr(p, 'premium_note') || 'N/A' },
    { key: 'accreditationNote', label: 'Accreditation detail', accessor: (p) => attrStr(p, 'accreditation_note') || 'N/A' },
  ...
  faq: [
    ...
    },
    {
      ...
      a: 'Every fee, accreditation and complaint reference on this page was researched against official sources and independent review platforms on 11 July 2026. Premiums over spot change with the live gold price, effectively daily, so always confirm a current quote directly with the provider before purchasing.',
    },
  ],
  compliance: {
    notice:
