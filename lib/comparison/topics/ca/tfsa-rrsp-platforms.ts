...
export const caTfsaRrspPlatformsConfig: TopicConfig = {
  ...
  metaDescription: (y) =>
    `Compare Canadian self-directed TFSA and RRSP brokerages of ${y} by commission, account fees and CIRO/CIPF protection, independent, expert-sourced.`,
  intro:
  ...
  compareRows: [
    ...
    { key: 'waiver', label: 'Fee waiver', accessor: (p) => attrStr(p, 'fee_waiver_note') || 'N/A' },
    { key: 'accounts', label: 'Account types', accessor: (p) => attrArr(p, 'account_types').join(', ') || 'N/A', score: (p) => attrArr(p, 'account_types').length },
    { key: 'research', label: 'Research tools', accessor: (p) => attrStr(p, 'research_tools_note') || 'N/A' },
