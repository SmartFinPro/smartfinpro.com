...
| **Multi-context item** | **Unit only** (`__tests__/unit/research-catalog-shell-logic.test.ts`, *"keeps a multi-context cockpit-only item as a single result and hrefs its first (manifest-order) context"*) | A `DiscoveryItem`'s `category` is singular, so a multi-context item requires ONE product qualifying in TWO topics **within the same category**. Every `BEST_X_MANIFEST` entry across all four live markets (us/uk/ca/au) was inspected (`lib/comparison/topics/manifest.ts`) before writing this row: no category currently has more than one topic anywhere in the manifest. There is no live product today that could honestly exercise this path — staging one on a real page would require fabricating catalog data, which the operator's instructions explicitly forbid. Fully covered by the existing synthetic-fixture unit test instead. |
