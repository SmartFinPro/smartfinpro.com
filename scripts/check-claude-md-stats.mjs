#!/usr/bin/env node
/**
 * scripts/check-claude-md-stats.mjs
 *
...
function checkCronNames(claudeMd) {
  const result = { ok: true, errors: [], warnings: [] };

  const cronIndex = readJson(path.join(GEN_DIR, 'cron-index.json'));
  if (!cronIndex) {
...
function checkKennzahlen(claudeMd) {
  const result = { ok: true, errors: [], warnings: [] };

  // Check cron count in Kennzahlen table
  const cronIndex = readJson(path.join(GEN_DIR, 'cron-index.json'));
  const actionsIndex = readJson(path.join(GEN_DIR, 'lib-actions.json'));
