#!/usr/bin/env tsx
/**
 * scripts/generate-route-index.ts
 *
...
interface RouteEntry {
  path: string;       // e.g. "/api/track"
  file: string;       // relative path from project root
  methods: string[];  // HTTP methods exported
}

function findRouteFiles(dir: string, base: string = ''): RouteEntry[] {
  ...
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      // Skip cron directory — handled by generate-cron-index.ts
